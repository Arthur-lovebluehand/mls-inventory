...
var _vendHideInactive = true;
async function vendors(){
  const{data:allData}=await sb.from('vendors').select('*').order('sort_order').order('name');
  const inactiveCount = (allData||[]).filter(v=>v.is_active===false).length;
  const data = _vendHideInactive ? (allData||[]).filter(v=>v.is_active!==false) : allData;
  $('main').innerHTML=`
  <div class="ph"><div><div class="pt">廠商管理</div><div class="ps">${allData?.length||0} 家</div></div>
  ...
  <div class="pc">
    <div style="margin-bottom:12px">
      <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px;color:var(--tx3)">
        <input type="checkbox" ${_vendHideInactive?'checked':''} onchange="_vendHideInactive=this.checked;vendors()">
        隱藏已停用${inactiveCount>0?`（${inactiveCount}家）`:''}
      </label>
    </div>
    <div class="tc">
    <div class="tb"><span class="tt">廠商列表</span></div>
    <div class="tw"><table style="width:100%">
      <tr><th style="text-align:center">排序</th><th>廠商編號</th><th>廠商名稱</th><th>聯絡人</th><th>電話</th><th>統一編號</th><th>付款方式</th><th>狀態</th><th>操作</th></tr>
      ${(data||[]).map(vd=>`<tr style="${vd.is_active===false?'opacity:.5':''}">
        ...
        <td style="font-size:11px;color:var(--tx3)">${vd.vendor_no}</td>
        <td style="font-weight:500;cursor:pointer" onclick="showVend('${vd.vendor_no}')">${vd.name}${vd.is_active===false?'<span class="badge br2" style="margin-left:5px;font-size:10px">停用</span>':''}</td>
        <td>${vd.contact||'—'}</td>
        ...
        <td><div style="display:flex;gap:3px">
          <button class="btn btn-s" onclick="showVend('${vd.vendor_no}')">明細</button>
          ...
          <button class="btn btn-s btn-r" onclick="deleteVendor('${vd.vendor_no}','${vd.name.replace(/'/g,"\\'")}')">刪</button>
        </div></td>
      </tr>`).join('')||`<tr><td colspan="9" style="text-align:center;padding:20px;color:var(--tx3)">${_vendHideInactive?'目前沒有啟用中的廠商':'尚無廠商資料'}</td></tr>`}
    </table></div>
  </div></div></div>`;
}

const VENDOR_PAYMENT_TERMS=['月結30天','月結60天','月結90天','貨到付款','預付','電匯即付','其他'];

function vendForm(vd){
  vd=vd||{};
  return `<div class="fg">
    <div class="fl"><label>廠商編號</label><input value="${vd.vendor_no||'（儲存後自動產生）'}" disabled style="opacity:.6;width:100%;padding:7px 8px;border:1px solid var(--bd);border-radius:var(--r);font-size:13px"></div>
    ${fi('vname','廠商名稱 *','text',vd.name)}
    ...
    ${payMethodSel('vpay',vd.payment_method||'')}
    <div class="fl"><label>付款條件</label><select id="f-vterm">${VENDOR_PAYMENT_TERMS.map(t=>`<option ${t===vd.payment_terms?'selected':''}>${t}</option>`).join('')}</select></div>
    ${fi('vbank','銀行名稱','text',vd.bank_name)} ${fi('vbacc','帳號','text',vd.bank_account)}
    ${fi('vbhld','戶名','text',vd.bank_holder)}
    <div class="fl fw">${fi('vaddr','登記地址（發票/統編用）','text',vd.full_address)}</div>
    <div class="fl fw">${fi('vshipaddr','實際收送地址（選填，若跟登記地址不同才填）','text',vd.ship_address)}</div>
...
async function genVendorNo(){
  const{data}=await sb.from('vendors').select('vendor_no').like('vendor_no','V-_____').order('vendor_no',{ascending:false}).limit(30);
  let max=0;
  (data||[]).forEach(r=>{ const m=(r.vendor_no||'').match(/^V-(\d{5})$/); if(m) max=Math.max(max,parseInt(m[1])); });
...
async function showVend(no){
  ...
  <div class="dg">
    ...
    <div class="dr"><span class="dlb">聯絡窗口</span><span class="dv">${vd?.contact||'—'}</span></div>
    <div class="dr"><span class="dlb">電話</span><span class="dv">${vd?.phone||vd?.mobile||'—'}</span></div>
    <div class="dr"><span class="dlb">傳真</span><span class="dv">${vd?.fax||'—'}</span></div>
    ...
    <div class="dr" style="grid-column:1/-1"><span class="dlb">銀行帳號</span><span class="dv">${[vd?.bank_name,vd?.bank_account,vd?.bank_holder].filter(Boolean).join(' / ')||'—'}</span></div>
    <div class="dr" style="grid-column:1/-1"><span class="dlb">登記地址</span><span class="dv">${vd?.full_address||'—'}</span></div>
    <div class="dr" style="grid-column:1/-1"><span class="dlb">實際收送地址</span><span class="dv">${vd?.ship_address||'（同登記地址）'}</span></div>
    ${vd?.note?`<div class="dr" style="grid-column:1/-1"><span class="dlb">備註</span><span class="dv">${vd.note}</span></div>`:''}
  </div>`,
  `<button class="btn" onclick="CM()">關閉</button>
   <button class="btn btn-p" onclick="CM();eVend('${no}')">編輯</button>`);
}
...
async function brands() {
  ...
  <div class="pc">
    ...
    <div class="tw"><table style="width:100%">
      <tr><th style="text-align:center">排序</th><th>品牌名稱</th><th>類型</th><th>產地/來源</th><th>官網</th><th>備註</th><th>狀態</th><th>操作</th></tr>
