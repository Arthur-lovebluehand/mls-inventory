...
async function loadPOForm(poData,itsData){
  ...
  if(itsData) _poItems=itsData.map((i,idx)=>({id:idx+1,pno:i.product_no,qty:i.qty,price:i.unit_price,amt:i.amount}));
...
function custForm(c){
  ...
  return `<div class="fg">
    ...
    ${fi('cpay','付款方式','text',c.payment_method||'付款交單 (D/P)')}
...
window.markBadDebt=markBadDebt; window.confirmBadDebt=confirmBadDebt; window.toggleBadDebt=toggleBadDebt;
...
window.brands=brands; window.addBrand=addBrand; window.editBrand=editBrand;
window.saveBrand=saveBrand; window.setBrandSort=setBrandSort;
window.toggleBrand=toggleBrand; window.deleteBrand=deleteBrand;
